...
int main(int argc , char *argv[]){
	...
	//Array pointer to save the documents to process
	word_document_str *docs_to_process = NULL;
	int doc_count;
	...
	if(rank==0){
		// Root Task
		//Declaring an array to save all the document names
		word_document_str documents[numDocs];
		int files_idx=0;
		// Passing through File directory once again to load to files array
		if((files = opendir("input")) == NULL){
			printf("Directory failed to open\n");
			exit(1);
		}
		while((file = readdir(files))!= NULL){
			...
			if(!strcmp(file->d_name, "."))	 continue;
			if(!strcmp(file->d_name, "..")) continue;
			strcpy(documents[files_idx],file->d_name);
			files_idx++;
		}
		// Distributing the documents rank wise and sending them in a round-robin way
		int send_rank=1;
		for(int d=0;d<files_idx;d++){
			MPI_Send(documents[d],1,mpi_string,send_rank,0,MPI_COMM_WORLD);
			send_rank++;
			if (send_rank==numproc)
				send_rank=1;
		}
	}
	else{
		// Worker Task
		// Declaring the number of documents to parse
		doc_count= (int)numDocs/(numproc-1);
		// Logic to distribute the remaining docs after equally dividing them 
		if(rank <= (numDocs%(numproc-1))) 
			doc_count++;
		// Allocating an array to store the documents to be processed by the worker
		docs_to_process = (word_document_str*) malloc(doc_count*sizeof(word_document_str));
		// Recieving the document names and saving them to docs_to_process
		for(int f=0;f<doc_count;f++){
        	MPI_Recv(&docs_to_process[f],1,mpi_string,0,0,MPI_COMM_WORLD,&status);
		}
	}
    ...
    if(rank!=0){
    	...
    	{
			strcpy(document, docs_to_process[i]);
